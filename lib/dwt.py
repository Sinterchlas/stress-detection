...
def compute_dwt_bandwidths(fs, levels=8):
    bands = []
    for j in range(1, levels + 1):
        ...
        bands.append({
            "Scale (j)": j,
            "Min Frequency (Hz)": f_min,
            "Max Frequency (Hz)": f_max,
...
def discrete_wavelet_transform():
    ...
    qj[j] = [0] * length
    for k in range(a, b):
        qj[j][k + abs(a)] = -2 * (dirac(k) - dirac(k+1))
    ...
    qj[j] = [0] * length
    for k in range(a, b):
        qj[j][k + abs(a)] = -1/4 * (dirac(k-1) + 3*dirac(k) + 2*dirac(k+1) 
    ...
    for k in range(a, b):
        ...
        qj[j][k + abs(a)] = (-1 / 32768*(eq1+eq2))

    delay = []
    for i in range(1, 9):
        delay.append(round(2 ** (i - 1)) - 1)
    return h, g, qj, delay
...
def dwt_freq_response(fs):
    h, g = filter_coefficients()
    Hw, Gw = hg_freq_response(fs, h, g)

    Q = np.zeros((9, fs // 2 + 1))
    for i in range(fs // 2 + 1):
        def safe(arr, idx): return arr[idx] if idx < len(arr) else 0

        Q[1][i] = safe(Gw, i)
        Q[2][i] = safe(Gw, 2*i) * safe(Hw, i)
        Q[3][i] = safe(Gw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
        Q[4][i] = safe(Gw, 8*i) * safe(Hw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
        Q[5][i] = safe(Gw, 16*i) * safe(Hw, 8*i) * safe(Hw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
        Q[6][i] = safe(Gw, 32*i) * safe(Hw, 16*i) * safe(Hw, 8*i) * safe(Hw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
        Q[7][i] = safe(Gw, 64*i) * safe(Hw, 32*i) * safe(Hw, 16*i) * safe(Hw, 8*i) * safe(Hw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
        Q[8][i] = safe(Gw, 128*i) * safe(Hw, 64*i) * safe(Hw, 32*i) * safe(Hw, 16*i) * safe(Hw, 8*i) * safe(Hw, 4*i) * safe(Hw, 2*i) * safe(Hw, i)
    return Q

def apply_dwt(delay, qj, total_time, sampling_rate, signal):
    # Use list indexing for w2fb initialization and updates
    length = int(total_time * sampling_rate)
    w2fb = [[0 for _ in range(length + max(delay))] for _ in range(9)]
    for n in range(length):
        for j in range(1, 9):
            for idx in range(1, 9):
                w2fb[idx][n + delay[idx-1]] = 0
            a = -(round(2**j) + round(2**(j-1)) - 2)
            b = -(a - round(2**(j-1)))
            for k in range(a, b+1):
                for idx in range(1, 9):
                    qj_idx = k + abs(a)
                    sig_idx = n - (k + abs(a))
                    if 0 <= qj_idx < len(qj[idx]) and 0 <= sig_idx < len(signal):
                        w2fb[idx][n + delay[idx-1]] += qj[idx][qj_idx] * signal[sig_idx]
    return w2fb

def pywt_dwt(signal, wavelet='db4', max_level=8):
    """
    Perform multi-level DWT using PyWavelets.
    Returns list of coefficients: [approximation, detail1, detail2, ...]
    """
